...
<nav class="sticky top-0 z-50 border-b border-white/5 bg-black/60 backdrop-blur-xl">
    <div class="max-w-7xl mx-auto flex justify-between items-center px-6 py-3">
        <a href={resolve('/')} class="group flex items-center gap-1 text-2xl font-black tracking-tighter text-blue-500 transition-transform active:scale-95" onclick={() => isMobileMenuOpen = false}>
        ...
        <div class="flex items-center">
            {#if user}
                <div class="hidden md:flex items-center gap-8">
                    <div class="flex items-center gap-6">
                        <a href={resolve('/hatch')} class="text-xs font-black uppercase tracking-widest text-gray-400 hover:text-white transition">Hatch</a>
                    ...
                    <div class="flex items-center gap-4 pl-8 border-l border-white/10">
                        <a href={resolve(`/profile/${user.id}`)} class="text-sm font-bold text-gray-300 hover:text-white transition">
                            {user.name}
                        </a>
            ...
            {:else}
                <a href={resolve('/login')} class="text-sm font-bold text-white bg-blue-600 px-6 py-2 rounded-xl hover:bg-blue-500 transition-all shadow-lg shadow-blue-600/20 active:scale-95">
                    Login
                </a>
            {/if}
        </div>
        ...
        <div class="absolute top-full left-0 w-full border-b border-white/5 bg-[#0A0A0A]/95 backdrop-blur-2xl md:hidden">
            <div class="flex flex-col px-6 py-6 space-y-6">
                <div class="flex flex-col space-y-4">
                    <a href={resolve('/hatch')} onclick={() => isMobileMenuOpen = false} class="text-lg font-black uppercase tracking-widest text-gray-400 hover:text-white transition active:scale-95 origin-left">Hatch</a>
                ...
                <div class="pt-6 mt-2 border-t border-white/10 flex items-center justify-between">
                    <a href={resolve(`/profile/${user.id}`)} onclick={() => isMobileMenuOpen = false} class="text-sm font-bold text-blue-400 hover:text-blue-300 transition uppercase tracking-widest">
                        My Profile
                    </a>
