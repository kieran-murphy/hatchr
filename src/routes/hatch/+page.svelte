...
<div class="flex flex-col items-center justify-center min-h-[70vh] gap-8 p-6">
    ...
    <div class="text-center">
        <h1 class="text-5xl font-black tracking-tighter text-white mb-2">THE NURSERY</h1>
        <p class="text-gray-500 font-medium italic">Cost: 10 Gems</p>
    </div>

    {#if form?.success && !hatching}
        <div in:scale={{ duration: 400, start: 0.8 }} class="flex flex-col items-center gap-4">
            ...
            <div class="text-center">
                ...
                <a href="/collection" class="text-sm text-blue-500 hover:underline mt-2 inline-block">View in Collection →</a>
            </div>
        </div>
    {:else}
        <form 
        ...
        >
            <button 
                ...
                class="group relative px-12 py-5 bg-blue-600 rounded-2xl font-black text-2xl overflow-hidden transition-all hover:scale-105 active:scale-95 disabled:opacity-50 disabled:grayscale"
            >
                <span class="relative z-10 flex items-center gap-3">
                    {#if hatching}
                        <span class="animate-spin">🥚</span> CRACKING...
                    {:else}
                        <span>🥚</span> HATCH EGG
                    {/if}
                </span>
