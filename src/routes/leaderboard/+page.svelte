...
<div class="max-w-3xl mx-auto py-12 px-6">
    <header class="mb-12 text-center">
        ...
        <p class="font-bold text-blue-500 uppercase text-sm tracking-widest mt-2">
                ...
                {#each data.leaderboard as user, index (user.id)}
                    <li>
                        <a 
                            ...
                            class="flex items-center justify-between p-4 rounded-2xl transition-all hover:-translate-y-1 group {getRankStyles(index)}"
                        >
                            <div class="flex items-center gap-6">
                                <div class="w-8 text-center font-black text-2xl italic {getRankText(index).split(' ')[0]}">
                                    #{index + 1}
                                </div>
                                ...
                                <div class="flex items-center gap-4">
                                    {#if user.image}
                                        <img 
                                            ...
                                            class="w-12 h-12 rounded-full object-cover border-2 {getRankText(index).split(' ')[1]} transition-colors"
                                        />
                                    {:else}
                                        <div class="w-12 h-12 bg-blue-500/10 text-blue-400 rounded-full flex items-center justify-center font-black text-xl border-2 {index < 3 ? getRankText(index).split(' ')[1] : 'border-blue-500/30'}">
                                            {user.username.charAt(0).toUpperCase()}
                                        </div>
                                    {/if}

                                    <span class="text-white font-black uppercase tracking-wider text-lg group-hover:text-blue-400 transition-colors">
                                        {user.username}
                                    </span>
                                </div>
                            </div>
                            
                            <div class="text-emerald-400 font-black tracking-widest bg-emerald-500/10 border border-emerald-500/20 px-4 py-2 rounded-xl flex items-center gap-2">
                                {user.uniqueCount} <span class="text-emerald-600/50 text-sm">/ 105</span> 
